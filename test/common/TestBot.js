...
const script = require('../../src/nashville-news');
...
class TestBotContext {
  constructor(robot, user) {
    ...
    this.robot.adapter.on('send', (_, strings) => {
      const value = strings.length === 1 && typeof strings[0] === 'object' ? strings[0] : strings.join('\n');
  ...
  shutdown() {
    delete process.env.HUBOT_NEWS_ITEM_COUNT;
    nock.cleanAll();
    this.robot.shutdown();
  }
}
